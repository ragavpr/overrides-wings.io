...
let Zb
  ...
  , bool_internet_explorer = -1 < str_browser_user_agent.indexOf("MSIE ") || 
...
function func_displayGameover() {
    ...
    $("#overlay").fadeIn(500);
    num_max_volume = .05 * num_setting_muteVol;
...
function func_millisToTimeFormat(total_millis) {
    ...
    : func_padZerosPrefix(hours) + ":" + func_padZerosPrefix(minutes) + ":" + func_padZerosPrefix(seconds)
}
...
function func_isInsideBox(x, y, dist) {
    ...
    return  x + dist >= d[0].x && x - dist <= d[1].x &&
            y + dist >= d[0].y && y - dist <= d[1].y ?
              true
            : false
}
...
window.stats = obj_browserQueryParams.stats ?
  true
: false;
"true" == window.localStorage.lq &&
  (bool_setting_highQuality = false);
window.mixpanel &&
...
window.clickPlay = function(b) {
    ...
    : (
        ...
        objG_eventManager.isSpaceWars() ?
          objGUI_gameInfo.showTip("Hint: Earn points faster by destroying asteroids.")
...
void 0 == num_setting_muteVol &&
  (num_setting_muteVol = 1);
"undefined" === typeof window.orientation ||
  int_pathMobile ||
    (window.location.href = "https://itunes.apple.com/us/app/wings.io/id1098205567?l=pt&ls=1&mt=8");
...
window.onblur = function() {
    ...
    objG_sfxManager &&
      ( objG_sfxManager.sound.volume(0),
        ...
        nb &&
          objG_sfxManager.sound.volume(0, nb)
      )
};
window.onfocus = function() {
    timeout_ws_conn &&
      ( clearTimeout(timeout_ws_conn),
        timeout_ws_conn = null
      );
    Lb = wa = true;
    num_max_volume = 1 * num_setting_muteVol;
    for (let b in objD_planes)
        objD_planes[b].clearTrail()
};
...
window.setInput = function(b, e, f) {
    ...
    !Sb &&
      f ? objG_wsConnection.sendShooting(f)
    : Sb && !f &&
...
window.connectionClosed = function() {
    ...
    int_pathMobile &&
      ( "undefined" != typeof messageHandlers &&
          messageHandlers.connectionClosed ?
...
if (int_pathMobile)
    window.onblur();
window.localStorage.nick && $("#nick")[0] &&
  ($("#nick")[0].value = window.localStorage.nick);
document.oncontextmenu = function() {
    return false
};
int_pathMobile &&
  (str_font_name = "Arial-BoldMT");
...
Class_TextStyle.prototype = {
    ...
    render: function() {
        ...
        if (this._dirty) {
            ...
            e.lineTo(b.width + this._angX - 4 * num_scale_factor, this._angY + 7 * num_scale_factor),
            e.lineTo(b.width + this._angX - 15 * num_scale_factor, b.height + this._angY - 9 * num_scale_factor),
            e.lineTo(this._angX + 5 * num_scale_factor, b.height + this._angY - 17 * num_scale_factor),
            e.fill());
            this._usingRoundedFrame && (e.globalAlpha = .3,
            func_drawRoundedRectangle(e, 0, 0, b.width, b.height, 40),
            e.globalAlpha = 1);
            int_pathMobile && (e.lineJoin = "round",
            e.lineWidth = 3,
...
let Class_EventManager = function() {
  ...
  , Class_Assets = function() {
    ...
    this.loadAnimations = function() {
        ...
        for (let f = 0; 13 > f; f++)
            ...
            obj_animation2.addFrame(this.frames["s" + f].renderTintedFrame("#2b9bf5"));
    ...
    this.loadWarshipEvent = function() {
        ...
        this.warshipImage.onload = function() {
            ...
            objG_assets.loadTintImage(objG_assets.warshipImage, function(b) {
    ...
    this.loadAsteroidEvent = function() {
        ...
        this.asteroidImage.onload = function() {
            ...
            objG_assets.loadTintImage(objG_assets.asteroidImage, function(b) {
  ...
  , Class_InputManager = function() {
    ...
    _this.keydown = function(event) {
        67 == event.keyCode && bool_following_plane && eb ? setTimeout(function() {
            func_displayCopyLink()
        }, 10) : bool_following_plane || (32 == event.keyCode ? Z && objG_wsConnection.sendShooting(true) : 188 != event.keyCode && 49 != event.keyCode && (222 == event.keyCode ? bool_drawUI = !bool_drawUI : 51 == event.keyCode ? (bool_drawGradient = !bool_drawGradient,
        console.log("Toggled Gradient to " + bool_drawGradient)) : 52 == event.keyCode ? (bool_drawClouds = !bool_drawClouds,
        console.log("Toggled drawClouds to " + bool_drawClouds)) : 53 == event.keyCode ? (bool_drawWater = !bool_drawWater,
        console.log("Toggled drawWater to " + bool_drawWater)) : 54 == event.keyCode ? (bool_drawExplosions = !bool_drawExplosions,
        console.log("Toggled drawExplosions to " + bool_drawExplosions)) : 55 == event.keyCode ? (bool_drawSun = !bool_drawSun,
        console.log("Toggled drawSun to " + bool_drawSun)) : 56 == event.keyCode ? (bool_drawItems = !bool_drawItems,
        console.log("Toggled drawItems to " + bool_drawItems)) : 57 == event.keyCode ? (bool_drawTrails = !bool_drawTrails,
        console.log("Toggled drawTrails to " + bool_drawTrails)) : 48 == event.keyCode ? (bool_drawSplashes = !bool_drawSplashes,
        console.log("Toggled drawSplashes to " + bool_drawSplashes)) : 27 == event.keyCode ? (func_showAds(),
        num_max_volume = .05 * num_setting_muteVol,
        $("#overlay").show(),
        funcR_hc_showPf(),
        bool_following_plane = true,
        Z || (ka = 1,
        objG_player_plane = null)) : 70 == event.keyCode && (document.fullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement || document.msFullscreenElement ? document.exitFullscreen ? document.exitFullscreen() : document.msExitFullscreen ? document.msExitFullscreen() : document.mozCancelFullScreen ? document.mozCancelFullScreen() : document.webkitExitFullscreen && document.webkitExitFullscreen() : document.documentElement.requestFullscreen ? document.documentElement.requestFullscreen() : document.documentElement.msRequestFullscreen ? document.documentElement.msRequestFullscreen() : document.documentElement.mozRequestFullScreen ? document.documentElement.mozRequestFullScreen() : document.documentElement.webkitRequestFullscreen && document.documentElement.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT),
        objGUI_gameInfo.clearTip())))
    };
    _this.keyup = function(event) {
        bool_following_plane || 32 != event.keyCode || (Z ? objG_wsConnection.sendShooting(false) : (1 == ka ? (ka = 0,
        objG_followMode.followTopPlayer()) : (ka = 1,
        objG_player_plane = null),
        objG_eventManager.isSpaceWars() && setTimeout(objG_followMode.respawnParticles, 500)))
    };
    ...
    _this.mouseup = function(event) {
        bool_following_plane || (Z ? 0 == event.button && objG_wsConnection.sendShooting(false) : bool_isLeftMousePressed = false)
    };
    _this.mousemove = function(event) {
        if (!bool_following_plane && (_this.mouseMoved = true,
        qc = event.clientX,
        rc = event.clientY,
  ...
  , Class_ParticleImpacts = function() {
    ...
    this.draw = function(e) {
        ...
        for (    f = [], d = 0; d < b.length; d++) {
            ...
            if (void 0 == objD_planes[a.id])
                ...
                e.stroke(),
                e.lineWidth = 4,
                e.strokeStyle = "rgba(150,255,150,1.0)") : a.weapon != id_weapon_trishoot ? (e.shadowColor = "red",
                e.lineWidth = 7,
    ...
    this.addShot = function(e, f, d, a) {
        if (func_isTimeElapsed_50ms() && xa) {
            ...
            if (c) {
                ...
                g.special = 0;
                objG_eventManager.isInstagib() ? g.special = 1 : objG_eventManager.isSpaceWars() && (g.special = 2);
                g.a = 1;
                g.weapon = a;
                ...
                c = g.hitPosition.x - e;
                f = g.hitPosition.y - d;
                a != id_weapon_superweapon && (a = Math.sqrt(c * c + f * f),
                g.direction = {
                    x: c / a,
                    y: f / a
                },
                ...
                g.finish = false,
                (func_isInsideBox(e, d, 100) || func_isInsideBox(g.hitPosition.x, g.hitPosition.y, 100)) && b.push(g));
                a = E / 2;
                g.hitPosition.y > a && (e = f / c,
                obj_particleImpacts.addSplash((a - (g.hitPosition.y - e * g.hitPosition.x)) / e, a + 6, 1, false))
            }
        }
    };
    ...
    this.addSplash = function(b, f, d, a) {
        if (bool_drawSplashes && !objG_eventManager.isSpaceWars() && func_isInsideBox(b, f, 100) && func_isTimeElapsed_50ms()) {
  ...
  , Class_UI_GameInfo = function() {
    ...
    this.draw = function(I) {
        ...
        if (0 < na)
            ...
            C && 0 < numG_player_count && (!bool_following_plane || bool_continueGame) && this.drawLeaderboard(I);
            if (objG_player_plane && !bool_following_plane) {
                I.save();
                int_pathMobile ? I.scale(1.5 * num_scale_factor, 1.5 * num_scale_factor) : I.scale(num_scale_factor, num_scale_factor);
            ...
            if (0 < d) {
                ...
                f ? (r = 40,
                int_pathMobile && (r = 50),
                q = new Class_TextStyle(r * num_scale_factor,"#FF0000","#990000"),
                q.setValue(f),
                q.setUsingFrame(true),
                r = true,
                int_pathMobile ? q.setAddTop(35) : q.setAddTop(25)) : (q = null,
                P = int_pathMobile ? 45 : 35),
                h = new Class_TextStyle(P * num_scale_factor,"#FF0000","#990000"),
                h.setValue(e),
                h.setUsingFrame(!r));
                P = .25 * canvas.height;
                a ? (h.setColor("#00ea11"),
            ...
            g || (r = 30,
            int_pathMobile && (r = 40),
            g = new Class_TextStyle(r * num_scale_factor,"#FF0000","#990000")),
            g.setValue(k),
            g.setUsingFrame(true),
            r = g.render(),
            O = .1 * Math.sin(m / 100) * num_scale_factor,
            I.save(),
            ...
            y || (r = 20,
            int_pathMobile && (r = 30),
            y = new Class_TextStyle(r * num_scale_factor,"#FF0000","#990000")),
            y.setValue(l),
            ...
            I.drawImage(r, 0, 0),
            I.restore());
            objG_wsConnection.hasConnection && xa && (objG_eventManager.waiting && !int_pathMobile ? this.DrawWarmupTime(I) : objG_eventManager.isInEvent() ? this.DrawEventLabel(I) : La && !bool_following_plane && this.DrawLaserDeployed(I));
            null == k && objUI_killStatus.shouldDraw() && (I.save(),
            I.translate(.5 * canvas.width, .67 * canvas.height),
            objUI_killStatus.draw(I),
            I.restore());
            Z || bool_following_plane || (O = "FREE MODE",
            0 == ka && (O = objG_player_plane ? "Following " + func_renameBlankPlayerNames(objG_player_plane.name) : "FOLLOW MODE"),
            c || (c = new Class_TextStyle(30 * num_scale_factor,"#ffd118","#b56006")),
            c.setValue(O),
            ...
            int_pathMobile && (r = 30),
            Ha = new Class_StrokeStyle(r * num_scale_factor,"#ffd118",true,"#633504"),
            Ha.setValue(Ta),
            Ha.setStrokeWidth(5),
            ...
            bool_following_plane || int_pathMobile || objUI_ActivityMessages.draw(I);
            !bool_following_plane && 0 < qa && this.DrawKing(I);
            ta && (ta = false);
            b != num_scale_factor && (c = g = null,
            b = num_scale_factor)
        }
    };
    this.DrawKing = function(a) {
        if (F && 0 < F.length && 0 < Object.keys(objD_planes).length) {
            let c = objD_planes[F[0]], d;
            if (c) {
                d = func_renameBlankPlayerNames(c.name);
                objG_player_plane && I != c.id && c.id == objG_player_plane.id && objG_sfxManager.playSound(str_sfxid_king, 1, 1, 1, null);
                I = c.id;
                d != Fa && (c = 25,
                int_pathMobile && (c = 35),
                ua = new Class_TextStyle(c * num_scale_factor,"#fe9b00","#6e3800"),
                c = " ",
            ...
            a.translate(.5 * canvas.width - E.width / 2 + c, d + E.height / 2 - 2 * num_scale_factor),
            d = 1,
    ...
    this.DrawRank = function(a) {
        0 < objG_player_plane.rank && (void 0 == rankCanvas && (rankCanvas = document.createElement("canvas"),
        rankCanvas.width = 200,
        rankCanvas.height = 200,
        rankCanvasContext = rankCanvas.getContext("2d")),
        lastRankNumber != objG_player_plane.rank && (rankCanvasContext.clearRect(0, 0, 200, 200),
        rankCanvasContext.globalAlpha = .3,
        rankCanvasContext.fillStyle = "rgba(0,0,0,1.0)",
        func_drawRoundedRectangle(rankCanvasContext, 0, 0, 200, 200, 30 * num_scale_factor),
    ...
    this.DrawScore = function(a) {
        if (0 < objG_player_plane.rank) {
            let c = 16 * num_scale_factor
              , d = "Arial Black";
            int_pathMobile && (c = 36 * num_scale_factor,
            d = "Arial-BoldMT");
            let b = P * num_scale_factor
              ...
              , n = Ka - b - 5
              , f = S + 5 + 5;
            int_pathMobile && (h = 60 * num_scale_factor,
            g = b);
    ...
    this.DrawWarmupTime = function(a) {
        if (U) {
            ...
            if (0 > parseInt(c))
                return;
            W = new Class_TextStyle(23 * num_scale_factor,objG_eventManager.getEventColor(),"#6e3800");
            W.setValue(objG_eventManager.getEventName());
            N = W.render();
            $_sub = new Class_TextStyle(20 * num_scale_factor,"#fe9b00","#6e3800");
    ...
    this.DrawEventLabel = function(a) {
        if (U) {
            ...
            let c = objG_eventManager.endTime - T;
            0 > parseInt(c) && (c = 0);
            W = new Class_TextStyle(25 * num_scale_factor,objG_eventManager.getEventColor(),"#6e3800");
            W.setValue(objG_eventManager.getEventName());
            N = W.render();
            Q = new Class_TextStyle(25 * num_scale_factor,"#fe9b00","#6e3800");
            objG_eventManager.isWarship() ? 0 < objG_eventManager.warshipsLeft ? Q.setValue("Warships Left: " + objG_eventManager.warshipsLeft) : 0 == objG_eventManager.warshipsEscaped ? Q.setValue("All Warships Destroyed!") : Q.setValue(objG_eventManager.warshipsDestroyed + " Destroyed, " + objG_eventManager.warshipsEscaped + " Escaped!") : Q.setValue(func_millisToTimeFormat(c));
            M = Q.render()
        }
        if (M) {
    ...
    this.DrawWinnerLabel = function(a) {
        !ia && aa && (ia = new Class_TextStyle(55 * num_scale_factor,"#fe6800","#6e3800"),
        ia.setValue("WINNER"),
        ha = ia.render(),
    ...
    this.DrawWarshipDestroyedLabel = function(a) {
        la || (la = new Class_TextStyle(45 * num_scale_factor,"#fe6800","#6e3800"),
    ...
    this.DrawLastEventWinner = function(a) {
        null != aa && aa != ja && (ca = new Class_StrokeStyle(15 * num_scale_factor,"#EEEEEE"),
        ca.setFont("px 'proxima-nova-1','proxima-nova-2', Arial Black"),
        ca.setValue("Last Event Winner: " + func_renameBlankPlayerNames(aa) + " "),
    ...
    this.renderLeaderboard = function(a, c) {
        P = 270;
        S = 0;
        let d = 5 * num_scale_factor, b = 10 * num_scale_factor, g = 23 * num_scale_factor, h = 18 * num_scale_factor, e, k = 60 * num_scale_factor, n = 5 * num_scale_factor, f = 32, q = "Arial Black", I = 0, l, r, y, m;
        int_pathMobile && (f = 42,
        P = 300,
        ...
        func_drawRoundedRectangleHalf(a, 0, 2, R, f * num_scale_factor, 30 * num_scale_factor);
        a.fillStyle = "rgba(254,102,3,1.0)";
        func_drawRoundedRectangleHalf(a, 0, 0, R, f * num_scale_factor, 30 * num_scale_factor);
        l = "LEADERBOARD";
            ...
            if (r = objD_planes[F[l]]) {
                for (n = 2; 0 <= n; n--) {
                    ...
                    if (r.inGame)
                        ...
                        y = 38,
                        9 == l && (y = 50),
                        a.translate(y * num_scale_factor * f + I, e + g + 7 * num_scale_factor * f),
    ...
    this.showTip = function(a) {
        int_pathMobile || (Ta && this.clearTip(),
        Ta = a,
        H = 0,
        Ia = null)
    };
  ...
  , Class_UI_ActivityMessages = function() {
    ...
    this.draw = function(e) {
        e.globalAlpha = 1;
        let d = canvas.height - 180 * num_scale_factor, a;
        for (a in b)
            e.drawImage(b[a], 25, d + 26 * num_scale_factor + 5 + 14 * a * num_scale_factor + 15 * a * num_scale_factor)
    };
  ...
  , Class_Plane = function() {
    ...
    this.update = function(b) {
        if (this.inGame) {
            ...
            func_isInsideBox(this.x, this.y, g) ? pa || (obj_particleTrails && obj_particleTrails.clear(),
            ...
            let k = func_clamp((T - this.lastUpdate) / num_global_physics_step_ms, 0, 1);
            this.prevX = this.x;
            this.prevY = this.y;
            ...
            obj_particleTrails && (obj_particleTrails.setPosition(k, m),
            objG_eventManager.isSpaceWars() && (obj_particleTrails.width = .6,
            this.id == qa && "#00FF00" != obj_particleTrails.style && (obj_particleTrails.style = "#00FF00")));
            obj_particleFlags && obj_particleFlags.setPosition(k, m);
            (k = !this.hover && !obj_particleManager && !this.isInvulnerable() && !g) ? 0 == this.hadHover && (this.hadHover = true,
            obj_particleTrails.push()) : this.hadHover = false;
            obj_particleTrails && (obj_particleTrails.enabled = k,
            obj_particleTrails.update(b));
            obj_particleFlags && (obj_particleFlags.enabled = k,
            obj_particleFlags.update(b));
            k = E / 2;
            m = k - 150;
            if (this.y > m && this.y < k && 0 >= h) {
                ...
                obj_particleImpacts.addSplash(this.x - l, k + 5 * Math.random(), m, true)
            }
            h -= b;
            obj_particleManager && (k = this.x + 12 * Math.sin(-this.angle),
            m = this.y + 12 * Math.cos(-this.angle),
            obj_particleManager.setPosition(k, m),
            obj_particleManager.update(b));
            objG_player_plane == this && (hb = 0 == ha ? hb + b : 0);
            2 != ha ? objD_planes[gb] && (w = Math.sqrt(hb / uc),
            1 < w && (w = 1),
            s -= b,
            0 >= s && (0 == ha ? (s = .7 > w ? 200 + s : 80 + s,
            objG_sfxManager.playSound(str_sfxid_lockon, .1, 1, const_Q_0, null)) : 1 == ha && (objGUI_gameInfo.showTargetLockedMessage(),
            s = 50 + s,
            objG_sfxManager.playSound(str_sfxid_lockon, .1, 1.58, const_Q_0, null)))) : (s = 0,
            objGUI_gameInfo.clearTargetLockedMessage());
            0 < ma ? (t -= b,
            0 > t && (t = 100,
            objG_sfxManager.playSound(str_sfxid_lockon, .1, .5, const_Q_0, null))) : t = 0;
            this == objG_player_plane && (ia || objG_sfxManager.playSound(str_sfxid_planeloop, .6, 1, const_Q_0, function(a) {
                ia = a
            }),
            na || objG_sfxManager.playSound(str_sfxid_waterloop, 0, 1, const_Q_0, function(a) {
                na = a
            }),
            k = (E / 2 - this.y) / 1E3,
            1 < k && (k = 1),
            ia && (m = objG_sfxManager.sound._nodeById(ia),
            objG_sfxManager.sound.volume(.5 * num_max_volume, ia),
            .01 > k && !objG_eventManager.isSpaceWars() ? m && m.bufferSource && (m.bufferSource.playbackRate.value = .1) : (l = this.speed / 30,
            1 < l && (l = 1),
            x += (.3 + .8 * l - x) / 30,
            m && m.bufferSource && (m.bufferSource.playbackRate.value = x))),
            na && (m = 0,
            .2 > k && .01 < k && !objG_eventManager.isSpaceWars() && (m = 1.7 * (.2 - k)),
            objG_sfxManager.sound.volume(m * num_max_volume, na)));
            g ? (k = func_calculateDistance2D(this.x, this.y, H.x, H.y),
            g = 1 - k / 3E3,
            1 < g && (g = 1),
            .1 > g && (g = 0),
            0 == Fa ? (objG_sfxManager.playSound(str_sfxid_lasershot, .2 * g, .5, const_Q_0, null),
            ua || objG_sfxManager.playSound(str_sfxid_laserloop, .2 * g, 1, const_Q_0, function(a) {
                nb = ua = a
            })) : ua && objG_sfxManager.sound.volume(.2 * g * num_max_volume, ua),
            this.laserTimer += b,
            Fa += b) : this.stopLaserSound();
            obj_particleManager && this.weapon == id_weapon_punch && 1 < this.ammo && (this.ammo = 1);
            0 < q && (q -= b);
    ...
    this.draw = function(b, h) {
        if (this.inGame && pa) {
            ...
            this.hover && (objG_eventManager.isSpaceWars() ? b.scale(.5, .5) : b.scale(.8, .8)),
            ...
            if (this.weapon == id_weapon_superweapon) {
                e = false;
                g > .5 * Math.PI && g < .5 * Math.PI + Math.PI || (e = true);
                if (this.isShooting) {
                    b.save();
                        K = this.x - 10 * Ga
                      , s = this.y - 10 * D
                    ...
                    ea ? objG_assets.frames["laser_collision" + this.laserFrame].draw(b) : (f = objG_assets.frames.laserfade.width,
                    b.rotate(Math.PI / 2),
                    b.translate(f / 2, 0),
    ...
    this.drawInput = function(a) {
        if (this.inGame) {
            ...
            a.fill();
            a.restore();
            2 != ha && (c = objD_planes[gb]) && this.DrawLockCrosshair(a, c.x, c.y, w, ha);
            0 < ma && this.DrawLockCrosshair(a, this.x, this.y, 1, 1);
            a.lineWidth = 1;
            a.beginPath();
            objG_eventManager.isSpaceWars() ? a.strokeStyle = "rgba(255,255,255,0.3)" : a.strokeStyle = "rgba(0,0,255,0.1)";
    ...
    this.drawInfo = function(a) {
        if (this.inGame && pa) {
            ...
            if (c || 0 < R || O || d)
                ...
                O ? objG_assets.frames.pause.draw(a) : c ? objG_assets.frames.crown.draw(a) : d ? objG_assets.frames.revengeIcon.draw(a) : objG_assets.frames.frenzyIcon.draw(a),
                a.restore();
            obj_scoreAccumInfo && (a.save(),
            d = 0,
            c && (d = 30),
    ...
    this.setPose = function(b, d, g, h) {
        ...
        objG_wsConnection.firstClientListing || (num_spawn_cooldown_ms = 1E3 * tc)) : (b = this.dstX - this.origX,
    ...
    this.setScore = function(a) {
        let c = a - this.score;
        0 < c && this == objG_player_plane && (obj_scoreAccumInfo || (obj_scoreAccumInfo = new Class_ScoreAccumInfo),
    ...
    this.setEnergy = function(a) {
        this.energy = a;
        25 > a && !obj_particleManager ? (obj_particleManager = new Class_ParticleManager,
        obj_particleManager.init(15, this.x, this.y),
        k = 0,
        m = 20 + 40 * Math.random(),
        objG_player_plane == this && (objG_sfxManager.playSound(str_sfxid_crash, .7, 1, const_Q_0, null),
        objGUI_gameInfo.clearNearMiss())) : 25 <= a && obj_particleManager && (delete obj_particleManager,
        obj_particleManager = null,
        l = 1)
    };
    ...
    this.stopLaserSound = function() {
        ua && (objG_sfxManager.sound.stop(ua),
        ua = nb = null)
    };
    this.dash = function() {
        this == objG_player_plane && objG_sfxManager.playSound(str_sfxid_woosh, .15, 1, const_Q_0, null);
        q = 300
    };
  ...
  , Class_SeaWater = function() {
    ...
    this.update = function(b) {
        ...
        if (l[1].y > E / 2) {
            ...
            q -= b;
            0 > q && (q = 125.6,
            20 > k && (m = new Class_SeaWaterRipple,
            h.push(m),
            l = l[1].x - l[0].x,
            y = 0,
            objG_player_plane && (y = 50 * objG_player_plane.getSpeedDirectionX()),
  ...
  , Class_Clouds = function() {
    ...
    (function() {
        for (let a = 0; a < b.length; a++) {
            let c = b[a], e = c.length, h = 9999, f = 9999, n = -9999, k = -9999, m = true;
            for (let l = 0; l < e; l++) {
                let y = c[l];
                m ? (y < h && (h = y),
                y > n && (n = y)) : (y < f && (f = y),
  ...
  , Class_Backgrounds = function() {
                    ...
                    for (; ; ) {
                        ...
                        if (0 == m)
                            ...
                            1 == f) {
                                ...
                                if (2 == f)
                                    for (f = c,
                                    l = f.getUint16(g, true),
                                    g += 2,
                                    y = 0; y < l; y++) {
                                        for (let r = f.getUint16(g, true), g = g + 2, p = new d, s = 0; s < r; s++) {
    ...
    this.drawGradient = function(a) {
        let c, b, d, f, m;
            c = (canvas.width / 2 + (objGUI_anchor.x * objGUI_anchor.zoom - canvas.width / 2)) / objGUI_anchor.zoom
          , b = (canvas.height / 2 + (objGUI_anchor.y * objGUI_anchor.zoom - canvas.height / 2)) / objGUI_anchor.zoom
          , d = .75 * E;
        objG_eventManager.isSpaceWars() && (d = E);
            d = a.createLinearGradient(0, -d, 0, d)
          , f = 39
          , m = 161;
        objG_eventManager.isSpaceWars() && (f = 0,
        m = 55);
        if (0 < e) {
            let l = e / 2500;
            1 < l && (l = 1);
        ...
        objG_eventManager.isSpaceWars() ? (d.addColorStop(0, "rgba(" + parseInt(f) + ",0,0,1.0)"),
        ...
        a.fillStyle = d;
        a.fillRect(c - canvas.width / 2 / objGUI_anchor.zoom, b - canvas.height / 2 / objGUI_anchor.zoom, canvas.width / objGUI_anchor.zoom, canvas.height / objGUI_anchor.zoom)
    };
    ...
    this.drawLimits = function(a) {
        let c = (canvas.width / 2 + (objGUI_anchor.x * objGUI_anchor.zoom - canvas.width / 2)) / objGUI_anchor.zoom
          , b = (canvas.height / 2 + (objGUI_anchor.y * objGUI_anchor.zoom - canvas.height / 2)) / objGUI_anchor.zoom
          , d = 500
          , e = 2 * E
          , f = 0
        ...
        if (0 < c) {
            ...
            l *= .2);
        objG_eventManager.isSpaceWars() && (d = 200,
        0 > b ? (p = E / 2,
        p += b,
        ...
        objG_eventManager.isSpaceWars() && (0 > b ? (a.fillStyle = "rgba(200,0,0," + y + ")",
        a.fillRect(-$_sub, -E / 2 - 2500, 2 * $_sub, 2500)) : (a.fillStyle = "rgba(200,0,0," + r + ")",
        a.fillRect(-$_sub, E / 2, 2 * $_sub, 2500)));
        objG_eventManager.isSpaceWars() || (a.fillStyle = "rgba(255,255,255,0.10)",
        a.fillRect(c - canvas.width / 2 / objGUI_anchor.zoom, -E / 2 - 1, canvas.width / objGUI_anchor.zoom, 6))
    };
    this.draw = function(a) {
        if (bool_drawSun && !objG_eventManager.isSpaceWars()) {
            let c = .97 * objGUI_anchor.x + 450
              , b = .97 * objGUI_anchor.y - 100
        ...
        bool_drawClouds && !objG_eventManager.isSpaceWars() && obj_clouds.drawPreRenderedClouds(a)
    };
    this.update = function(a) {
        if (xa || !c)
            if (c = true,
            ...
            null != objG_player_plane) {
                ...
                objG_eventManager.isSpaceWars() && (b |= objG_player_plane.y > E / 2 || objG_player_plane.y < -E / 2);
                b ? (0 == e && (objG_sfxManager.playSound(str_sfxid_warn, 1, 1, 1, null),
                objGUI_gameInfo.showWarningMessage("YOU ABANDONED THE FIGHT!")),
                e += a) : (0 < e && objGUI_gameInfo.clearWarningMessage(),
                e = 0)
            }
    };
}
  , Class_PickupItem = function() {
    ...
    this.update = function(f) {
        ...
        if (64 != this.type) {
            ...
            this.fadingOut && (y || objG_eventManager.isSpaceWars() || objG_eventManager.isInstagib() ? delete objD_pickups[this.id] : (e += .25,
            this.alpha -= f / 1E3,
            0 >= this.alpha && delete objD_pickups[this.id]))
        }
        this.grabbing && ((f = objD_planes[c]) ? (s = Math.pow(g, 2),
        this.x = h + (f.x - h) * s,
        this.y = q + (f.y - q) * s,
        n = 1 - s,
        g += .07 * p,
        1 < g && delete objD_pickups[this.id]) : delete objD_pickups[this.id])
    };
    this.drawItem = function(c, b) {
        ...
        if (64 == this.type)
            ...
            objG_player_plane && objG_eventManager.isSpaceWars() && objG_player_plane.id == qa && (g = .3),
            c.save(),
            c.beginPath(),
            this.sinScaleX += .1,
            this.sinScaleY += .12,
            ...
            g = 1;
            !objG_player_plane || objG_player_plane.weapon != id_weapon_superweapon || this.type != id_weapon_trishoot && this.type != id_weapon_missile && this.type != id_weapon_railgun && this.type != id_weapon_punch && this.type != id_weapon_bombs || (g = .3);
    ...
    this.draw = function(a) {
        if (bool_drawItems) {
            let c = 40, b;
            32 == this.type ? b = objG_assets.frames.health : this.type == id_weapon_trishoot ? b = objG_assets.frames.trishoot : this.type == id_weapon_railgun ? b = objG_assets.frames.railgun : this.type == id_weapon_missile ? b = objG_assets.frames.missile : this.type == id_weapon_punch ? b = objG_assets.frames.melee : this.type == id_weapon_bombs ? b = objG_assets.frames.bombdrop : this.type == id_weapon_superweapon ? b = objG_assets.frames.laser : 64 == this.type && (c = 10,
            b = objG_assets.frames.wing);
            let d = this.y;
            this.grabbing || (d += e);
            if (!this.grabbing) {
                if (!func_isInsideBox(this.x, d, c))
                    return
            } else if (!func_isInsideBox(this.x, this.y, c))
                return;
            this.drawItem(a, b)
        }
    };
    ...
    this.playerGrab = function(a) {
        ...
        objG_player_plane && c == objG_player_plane.id && !objG_eventManager.isInstagib() && (64 == this.type ? objG_player_plane.incScore(vc) : objG_player_plane.incScore(wc))
    };
  ...
  , Class_WS_Connection = function() {
    let f;
    function b(b, a, c) {
        169 != a && 172 != a && 162 != a && 178 != a || c || (list_str_leaderboard_players = []);
        let e, h, f, n, k, m, l, y, r, p, s, u, w, t, x, z, C, P, R, S, O, G, ea;
            e = false
          , h = 1
          , f = 1
          , n = 0;
        c || (objG_wsConnection.lastUpdateBool = !objG_wsConnection.lastUpdateBool);
        for (; ; ) {
            n++;
                k = b.getUint32(h, true)
              , h = h + 4;
            if (0 == k) {
                c = b;
                f = a;
                ...
                for (k = 0; k < n; k++) {
                    ...
                    if (null == s) {
                        ...
                        if (m) {
                            ...
                            .01 < t && objG_sfxManager.playSound(str_sfxid_cannonshoot, t, 1, w, null),
                            m.cannonShoot()) : (objG_player_plane && p == objG_player_plane.id || (w = const_Sa_3,
                            u = func_calculateDistance2D(m.x, m.y, H.x, H.y),
                            t = 1 - u / num_sound_max_distance),
                            .01 < t && objG_sfxManager.playSound(str_sfxid_mlaunch, t, 1, w, null),
                            m.ammo--)
                        }
                    }
                    s.lastUpdate = T;
                ...
                if (162 == a || 178 == a || 163 == a || 179 == a || 169 == a || 172 == a)
                    for (l = h; ; ) {
                        ...
                        y = b.getUint8(l, true);
                        l += 1;
                        h.type == id_entity_warship ? (r = -b.getFloat32(l, true),
                        l += 4,
                        p = b.getFloat32(l, true),
                        l += 4,
                        h.setCannonAngle(p),
                        h.setFloatValue(r)) : h.type != id_entity_asteroid || 162 != a && 178 != a || (r = b.getUint8(l, true),
                        l += 1,
                        h.setFragment(r));
                        h.setState(y);
                        h.setPose(c, f, n);
            ...
            objG_player_plane == C)) {
                  ...
                  , O = x & 128
                  , G = x & 4096;
                x & 64 ? objGUI_gameInfo.addBonus(64, 0) : S ? objGUI_gameInfo.addBonus(32, 0) : R ? objGUI_gameInfo.addBonus(16, 0) : z && objGUI_gameInfo.addBonus(8, 0);
                G && (objGUI_gameInfo.addBonus(4096, 0),
                Za[C.id] = 0);
            ...
            if (162 != a && 178 != a || !y)
                ...
                162 == a ? (x = b.getUint16(h, true),
                h += 2) : (x = b.getUint8(h, true),
                h += 1);
    ...
    function e(b, a) {
        let c, e, f, q, n, k;
            c = 1, e = b.getUint16(c, true), c = c + 2, f = b.getUint32(c, true), c = c + 4, q;
        168 == a ? (q = b.getUint8(c, true),
        c += 1) : (q = b.getUint16(c, true),
        c += 2);
        if (0 < f) {
                n = 1
              , k = str_sfxid_weapgrab;
            32 > q || 128 == q || 256 == q ? (c = b.getUint16(c, true),
            objD_planes[f].setWeapon(q),
            objD_planes[f].ammo = c) : 64 == q ? (objD_planes[f].trailEffect(),
            k = str_sfxid_winggrab,
            null != this.lastGrabbedWingTime && (q = T - this.lastGrabbedWingTime,
            1E3 > q ? objG_wsConnection.wingsInARow++ : objG_wsConnection.wingsInARow = 0,
            n = 1 + .05 * objG_wsConnection.wingsInARow,
            1.5 < n && (n = 1.5)),
            this.lastGrabbedWingTime = T) : 32 == q && (k = str_sfxid_hgrab);
            objG_player_plane && f == objG_player_plane.id && objG_sfxManager.playSound(k, 1, n, const_Q_0, null);
            objD_pickups[e].playerGrab(f)
        } else
            objD_pickups[e].fadeOut()
    }
    ...
    this.getServerAndConnect = function() {
        ...
        if (!int_pathMobile) {
            ...
            if (void 0 == a)
                setTimeout(objG_wsConnection.getServerAndConnect, 200);
            else {
                    c = "";
                bool_isHttps && (c = "s");
                $.ajax({
                    ...
                    success: function(a) {
                        if ("0" == a)
                            ...
                            a = c.split("/");
                            objG_wsConnection.roomNumber = 0;
                            1 < a.length && (objG_wsConnection.roomNumber = a[1],
                            c = a[0]);
    ...
    this.connect = function() {
        ...
        if (wa || int_pathMobile) {
            ...
            if (bool_isHttps) {
                ...
                0 < objG_wsConnection.roomNumber && (b = "ws:" + b.split(":")[1] + ":" + (parseInt(objG_wsConnection.roomNumber) + 8079),
            ...
            setTimeout(objG_wsConnection.getServerAndConnect, 100)
    };
    this.disconnect = function() {
        objG_wsConnection.directed && (func_isIframe() || (window.location.hash = ""),
    ...
    this.hello = function() {
        int_pathMobile ? objG_wsConnection.sendSingleByte(32) : objG_wsConnection.sendSingleByte(1);
        ...
        $("#nick").focus();
        1 == int_pathMobile && ("undefined" != typeof messageHandlers && messageHandlers.didConnect ? messageHandlers.didConnect(JSON.stringify({})) : window.webkit.messageHandlers.didConnect.postMessage({}))
    };
    this.processMessage = function(d) {
        ...
        if (0 != a)
            if (160 == a) {
                ...
                xa = true,
                int_pathMobile && ("undefined" != typeof messageHandlers && messageHandlers.didEnterGame ? messageHandlers.didEnterGame(JSON.stringify({})) : window.webkit.messageHandlers.didEnterGame.postMessage({})),
                c = 1,
                ...
                164 == a ? b(d, 162, true) : b(d, 178, true),
                func_setPlayerCount();
            else if (165 == a) {
                q = 1;
                ...
                if (c = objD_planes[a])
                    ...
                    q = objG_player_plane,
                    !q && Qb && (q = Qb),
                    q && f == q.id && (n = func_renameBlankPlayerNames(objD_planes[a].name),
                    objGUI_gameInfo.addMessage("You killed", true, n),
                    objG_eventManager.isInstagib() || (a == qa ? q.incScore(yc) : q.incScore(xc))),
                    0 < f ? ((f = objD_planes[f]) ? (objGUI_gameInfo.addActivityMessage(func_renameBlankPlayerNames(c.name) + " was Killed by " + func_renameBlankPlayerNames(f.name)),
                    f.incKills(),
                    Za[c.id] = f.id) : objGUI_gameInfo.addActivityMessage(func_renameBlankPlayerNames(c.name) + " was Killed"),
                    objG_animationManager.addExplosion(c.dstX, c.dstY, c.getSpeedDirectionX(), c.getSpeedDirectionY()),
                    objG_player_plane && objG_player_plane.id == a && (f ? objGUI_gameInfo.addMessage("Killed by", false, func_renameBlankPlayerNames(f.name)) : objGUI_gameInfo.addMessage("You were killed", false, null),
                    Z ? (Z = false,
                    Qb = objG_player_plane,
                    objG_player_plane = null,
                    ka = 1,
                    wasKilled(d),
                    func_displayGameoverScore(d)) : (objG_player_plane = null,
                    objG_followMode.waitUntilNextFollow()))) : (Za[c.id] = 0,
                    objG_player_plane && objG_player_plane.id == a && (5 == h ? objGUI_gameInfo.addMessage("You left!", false, null) : 4 == h ? objGUI_gameInfo.addMessage("Abandoned!", false, null) : 1 == h ? objGUI_gameInfo.addMessage("Crashed!", false, null) : 6 == h ? objGUI_gameInfo.addMessage("Exploded!", false, null) : 7 == h ? objGUI_gameInfo.addMessage("KAMIKAZE!!", false, null) : 8 == h ? objGUI_gameInfo.addMessage("You were killed by a Warship!", false, null) : 3 == h ? (objGUI_gameInfo.addMessage("Splash!", false, null),
                    objG_sfxManager.playSound(str_sfxid_bigsplash, .7, 1, const_Q_0, null),
                    obj_particleImpacts.addSplash(objG_player_plane.x, E / 2 + 10, 2, false)) : objGUI_gameInfo.addMessage("You were killed!", false, null),
                    Z ? (Z = false,
                    objG_player_plane = null,
                    ma = 0,
                    ka = 1,
                    ...
                    func_displayGameoverScore(d)) : (objG_player_plane = null,
                    objG_followMode.waitUntilNextFollow())),
                    5 == h ? objGUI_gameInfo.addActivityMessage(func_renameBlankPlayerNames(c.name) + " left") : 4 == h && objGUI_gameInfo.addActivityMessage(func_renameBlankPlayerNames(c.name) + " abandoned"),
                    objG_animationManager.addExplosion(c.dstX, c.dstY, .5 * c.getSpeedDirectionX(), .5 * c.getSpeedDirectionY())),
                    null == c ? console.log("ERROR: Trying to remove a player that didnt exist!") : objD_planes[a].cleanup();
                else if (d = objD_specialEntities[a])
                    d.type == id_entity_asteroid && objG_eventManager.isSpaceWars() && (objG_animationManager.addExplosion(d.x, d.y, 0, 0),
                    3 != d.fragment && (d = 1 - func_calculateDistance2D(d.x, d.y, H.x, H.y) / num_sound_max_distance,
                    .01 < d && objG_sfxManager.playSound(str_sfxid_mexpl2, d, .4, const_Sa_3, null))),
                ...
                h = 0; h < f; h++) {
                    ...
                    c += 4;
                    176 == a ? (q = d.getUint16(c, true),
                    c += 2) : (q = d.getUint8(c, true),
                    c += 1);
                    m = 1;
                    l = str_sfxid_empty;
                    switch (q) {
                    case id_weapon_machinegun:
                        l = objG_eventManager.isSpaceWars() ? qa == n ? str_sfxid_kinglaser : str_sfxid_laser : str_sfxid_shot;
                        break;
                    case id_weapon_trishoot:
                        objG_eventManager.isSpaceWars() ? (m = .7,
                        l = str_sfxid_laser) : l = str_sfxid_trishot;
                        break;
                    case id_weapon_railgun:
                        objG_eventManager.isSpaceWars() ? (l = str_sfxid_laser,
                        m = 1.3) : l = str_sfxid_rail
                    }
                    k = objD_planes[n];
                    !k && 0 < n && (y = objD_specialEntities[n]) && console.log("specialEntity did shoot: " + y.id);
                    y = const_Q_0;
                    r = 1;
                    objG_player_plane && n == objG_player_plane.id || !k || (y = const_Sa_3,
                    r = 1 - func_calculateDistance2D(k.x, k.y, H.x, H.y) / num_sound_max_distance);
                    .01 < r && l != str_sfxid_empty && objG_sfxManager.playSound(l, r, m, y, null);
                    ...
                    for (y = 0; y < l; y++) {
                        ...
                        for (x = false; ; ) {
                            ...
                            z == qa || ta || (t = false);
                            Ca && (Ca.hit(q),
                            k && q == id_weapon_superweapon && (x = true,
                            k.laserHit(r, s, true)),
                            objG_player_plane && (objG_player_plane.id == z ? objG_sfxManager.playSound(str_sfxid_phit, 1, 1, const_Q_0, null) : n == objG_player_plane.id && objG_sfxManager.playSound(str_sfxid_phit, 1, 2, const_Q_0, null)))
                        }
                        k && !x && k.laserHit(r, s, false);
                        t && obj_particleImpacts.addMissileImpact(r, s)
                    }
                    0 < n && (n = objD_planes[n]) && q != id_weapon_missile && q != id_weapon_bombs && (n.ammo = m,
                    n.setWeapon(q))
                }
            else if (167 == a || 174 == a)
                for (c = 1; ; ) {
                    ...
                    if (0 == f)
                        break;
                    c += 2;
                    174 == a ? (h = d.getUint16(c, true),
                    c += 2) : (h = d.getUint8(c, true),
                    c += 1);
                    q = d.getFloat32(c, true);
                    c += 4;
                    n = -d.getFloat32(c, true);
                ...
                a = 1,
                c = d.getUint8(a, true),
                3 == c ? ma++ : 4 == c ? ma-- : (gb = 2 != c ? d.getUint32(a + 1, true) : 0,
                hb = 0,
                ha = c);
            else if (16 == a)
                c = d.getUint8(1, true),
                ...
                objG_eventManager.type == objG_eventManager.eventType.EVENT_WARSHIP && (h = d.getUint8(c, true),
                c++,
                f = d.getUint8(c, true),
                c++,
                d = d.getUint8(c, true),
                ...
                objG_eventManager.endTime = +new Date + 1E3 * a,
                objG_eventManager.isInstagib() ? objG_eventManager.railSwitch = true : objG_eventManager.isWarship() ? objG_assets.loadWarshipEvent() : objG_eventManager.isSpaceWars() && objG_assets.loadAsteroidEvent();
            else if (20 == a) {
                objG_eventManager.type != objG_eventManager.eventType.EVENT_WARSHIP && (objG_followMode.whiteFlash = 1);
                objG_eventManager.waiting = false;
                ...
                objG_eventManager.waiting = false,
                objG_eventManager.type != objG_eventManager.eventType.EVENT_NONE && (objG_eventManager.type != objG_eventManager.eventType.EVENT_WARSHIP && (objG_followMode.whiteFlash = 1),
                objG_eventManager.setType(objG_eventManager.eventType.EVENT_NONE));
            else if (23 == a) {
                c = 1;
                f = d.getUint32(c, true);
                ...
                if (0 < f) {
                    ...
                    objGUI_gameInfo.setWarshipRemoved(null);
                a = d.getUint8(c, true);
                c += 1;
                f = d.getUint8(c, true);
                c += 1;
    ...
    this.sendInput = function() {
        ...
        let c = 0;
        if (objG_inputManager.hover || !wa)
            c |= 1;
        if (!wa || bool_following_plane)
            c |= 2;
        a.setUint8(9, c, true);
        f.send(b)
    };
  ...
  , ClassR_FollowMode = function(b) {
    ...
    _this.followTopPlayer = function() {
        ...
        b && (objG_player_plane = objD_planes[b],
    ...
    _this.update = function(a) {
        ...
        if (objG_assets.loaded) {
            objG_player_plane && !objG_player_plane.inGame && 0 == ka && (objG_player_plane = null,
            ra = 0);
            ...
            for (b in objD_pickups)
                ...
                objD_pickups[b] && !La && objD_pickups[b].type == id_weapon_superweapon && (La = true,
                Ma = null);
            e = null;
            for (b in objD_missiles)
                objD_missiles[b].update(a),
                objD_missiles[b].finished && (e = b);
            e && delete objD_missiles[e];
            e = null;
            for (b in objD_planes)
                objD_planes[b].updateBool != objG_wsConnection.lastUpdateBool ? (objD_planes[b] == objG_player_plane && (ra = 0),
                delete objD_planes[b]) : (!La && objD_planes[b].weapon == id_weapon_superweapon && objD_planes[b].inGame && (La = true,
                Ma = func_renameBlankPlayerNames(objD_planes[b].name)),
                sc == b && objD_planes[b].inGame && (e = objD_planes[b]),
                objG_eventManager.railSwitch ? (objD_planes[b].setWeapon(id_weapon_railgun),
                objD_planes[b].ammo = -1) : objG_eventManager.machinegunSwitch && (objD_planes[b].setWeapon(id_weapon_machinegun),
                objD_planes[b].ammo = -1),
                objD_planes[b].update(a));
            for (b in objD_specialEntities)
                objD_specialEntities[b].update(a);
            objG_eventManager.railSwitch && (objG_eventManager.railSwitch = false);
            objG_eventManager.machinegunSwitch && (objG_eventManager.machinegunSwitch = false);
            objGUI_anchor.update(a);
            if (!Z && 0 == ka && null == objG_player_plane && (c -= a,
            0 >= c))
                for (b in objD_planes) {
                    objD_planes[b].inGame && (objG_player_plane = objD_planes[b],
                    objG_player_plane.prepareFollow(),
                    ra = b);
                    break
                }
            if (objG_eventManager.isSpaceWars()) {
                for (i in list_particleDust) {
                    list_particleDust[i].update(objGUI_anchor.getBounds(), objGUI_anchor.zoom);
            ...
            H = {
                x: (html_canvas.width / 2 + (objGUI_anchor.x * objGUI_anchor.zoom - html_canvas.width / 2)) / objGUI_anchor.zoom,
                y: (html_canvas.height / 2 + (objGUI_anchor.y * objGUI_anchor.zoom - html_canvas.height / 2)) / objGUI_anchor.zoom
            };
            ...
            if (!int_pathMobile && null != objG_player_plane)
                if (objG_inputManager.mouseMoved) {
                    ...
                    Zb = (qc + (objGUI_anchor.x * objGUI_anchor.zoom - html_canvas.width * a / 2)) / objGUI_anchor.zoom;
                    $b = (rc + (objGUI_anchor.y * objGUI_anchor.zoom - html_canvas.height * a / 2)) / objGUI_anchor.zoom;
            ...
            if (e && !func_isInsideBox(e.x, e.y, 30)) {
                ...
                q = false;
                E, w = false;
            (ub = objG_player_plane ? Za[objG_player_plane.id] : 0) && 0 < ub && (E = objD_planes[ub],
            E == e && (w = true));
            !E || func_isInsideBox(E.x, E.y, 30) || w ? y = false : (w = H.x - E.x,
            D = H.y - E.y,
            E = Math.sqrt(w * w + D * D),
            r = 1,
            1300 < E && (r = 1300 / E,
            ...
            if (objG_eventManager.isWarship()) {
                ...
                0 < e ? func_isInsideBox(Q, I, 120) ? T = false : (1300 < E && (Ca = 900 / E,
                .3 > Ca && (Ca = .3)),
                D = Y / N,
                C = H.y - D * H.x,
                L = 0 > N ? D * a + C : D * f + C,
    ...
    _this.draw = function(b) {
        if (objG_assets.loaded) {
            ...
            if (objG_eventManager.isSpaceWars())
                for (i in list_particleDust)
                    list_particleDust[i].draw(ctx_canvas);
            else
                c > E / 2 - 25 && (e = true,
                objG_backgrounds.drawWaterBehind(ctx_canvas));
            if (xa) {
                for (f in objD_pickups)
                    objD_pickups[f].draw(ctx_canvas);
                obj_particleImpacts.draw(ctx_canvas);
                for (f in objD_missiles)
                    objD_missiles[f].draw(ctx_canvas, b);
                for (f in objD_planes)
                    objD_planes[f].draw(ctx_canvas, b);
                for (f in objD_planes)
                    objD_planes[f].drawInfo(ctx_canvas);
            ...
            if (xa) {
                if (!objG_eventManager.isSpaceWars()) {
                    for (f in objD_planes)
                        objD_planes[f].drawReflection(ctx_canvas, b);
                    for (f in objD_missiles)
                ...
                ctx_canvas.rotate(-k),
                e = 1.2,
                c ? (ctx_canvas.translate(0, -(-20 - 10 * b)),
                e = .9) : ctx_canvas.translate(0, -(-30 - 10 * b)),
                ...
                ctx_canvas.translate(e.x, e.y),
                ctx_canvas.drawImage(objG_assets.warshipIcon, -objG_assets.warshipIcon.width / 2, -objG_assets.warshipIcon.height / 2),
                ctx_canvas.restore(),
            ...
            0 < this.whiteFlash && (ctx_canvas.save(),
    ...
    _this.gameCleanup = function() {
        ...
        for (id in objD_pickups)
            delete objD_pickups[id];
        objD_pickups = {};
        for (id in objD_missiles)
            delete objD_missiles[id];
        objD_missiles = {};
        for (id in objD_planes)
            delete objD_planes[id];
        objD_planes = {};
        for (id in objD_specialEntities)
            delete objD_specialEntities[id];
        objD_specialEntities = {}
    };
    _this.resize = function(a) {
        C();
        objG_eventManager.isSpaceWars() && (N && clearTimeout(N),
        N = setTimeout(_this.respawnParticles, 200))
    };
    let C = function() {
        let a = 2, b, c;
        ...
        num_scale_factor = .92 * Math.max(ib / 1016, Ka / 1500);
    ...
    objG_assets.load(function() {
        ...
        1 == int_pathMobile && ("undefined" != typeof messageHandlers && messageHandlers.didLoad ? messageHandlers.didLoad(JSON.stringify({})) : window.webkit.messageHandlers.didLoad.postMessage({}));
        objGUI_gameInfo = new Class_UI_GameInfo;
        obj_particleImpacts = new Class_ParticleImpacts;
        objG_wsConnection.hasConnection && !objG_wsConnection.sentHello && objG_wsConnection.hello();
        setInterval(e, 40);
  ...
  , Class_UI_Anchor = function(html_canvas, ctx_canvas, x, y) {
    ...
    this.update = function(b) {
        ...
        xa || (_this.y = 470);
        Z || 1 != ka || bool_following_plane ? (_this.zoom = 1 / (window.devicePixelRatio / int_pixel_ratio) * 1.2,
        _this.zoom *= num_scale_factor,
        int_pathMobile && (_this.zoom += .3),
        null != objG_player_plane && (ca.x = objG_player_plane.x - _this.x,
        ca.y = objG_player_plane.y - _this.y,
        _this.x = objG_player_plane.x,
        _this.y = objG_player_plane.y)) : (c = 1 / (window.devicePixelRatio / int_pixel_ratio) * .7 * num_scale_factor,
  ...
  , Class_AnimationManager = function() {
    ...
    this.addBlast = function(a, b, d, e, f) {
        ...
        .01 < a && objG_sfxManager.playSound(str_sfxid_mexpl2, a * f, 1, e, null)
    };
    this.addExplosion = function(b, d, e, f) {
        if (bool_drawExplosions && func_isInsideBox(b, d, 100) && func_isTimeElapsed_50ms()) {
            if (1 >= a.length) {
  ...
  , Class_ParticleManager = function() {
    ...
    this.update = function(b) {
        ...
        for (e = 0; e < d; e++) {
            g = f[e];
            g.time >= this.life && (g.active || (g.active = true,
            g.time %= this.life),
  ...
  , Class_Missile = function() {
    ...
    this.type = 0;
    this.update = function(d) {
        let a = func_clamp((T - this.lastUpdate) / num_global_physics_step_ms, 0, 1)
          , c = this.y - this.prevY
          , g;
        0 == this.x - this.prevX && 0 == c && (f++,
        50 < f && (this.finished = true));
        this.prevX = this.x;
        this.prevY = this.y;
        ...
        if (1 == this.type)
            objG_eventManager.isSpaceWars() && (this.angle += .2);
        else if (2 != this.type) {
            a = E / 2;
            c = a - 100;
            if (this.y > c && this.y < a && 0 >= e) {
                ...
                obj_particleImpacts.addSplash(this.x - g, a + 5 * Math.random(), c, true)
            }
            e -= d
        }
    };
    this.draw = function(d, a) {
        ...
        let e = .7;
        this.flameState && (e = 1);
        0 == this.type && (obj_particleTrails.width = 1.5 * e,
        obj_particleTrails.style = "rgba(247, 189, 57, 1.0)",
        obj_particleTrails.draw(d),
        obj_particleTrails.width = .4 * e,
        ...
        d.scale(c, c),
        1 == this.type && objG_assets.blinkImage && objG_eventManager.isSpaceWars() && (d.scale(2, 2),
        d.drawImage(objG_assets.blinkImage, -objG_assets.blinkImage.width / 2, -objG_assets.blinkImage.height / 2),
        d.scale(.5, .5)),
        d.rotate(this.angle),
        0 == this.type ? objG_assets.frames.missile_attack.draw(d) : objG_assets.frames.bomb.draw(d)) : (d.beginPath(),
        d.fillStyle = "#FF0000",
    ...
    this.setPosition = function(b, a, c) {
        ...
        this.dstX = 10 * b;
        this.dstY = 10 * a;
        this.first_set && ((b = objD_planes[c]) ? (a = 5 * Math.abs(Math.cos(b.angle + Math.PI / 2)),
        this.origX = b.x,
        this.origY = b.y + a,
        this.x = b.x,
  ...
  , Class_ParticleFlags = function() {
    ...
    this.setTexture = function(b) {
        ...
        this.texture.onload = function() {
            ...
            let a = _this.scale;
            _this.flagHeight = .5 * _this.textureHeight * a;
    ...
    this.draw = function(f) {
        if (bool_drawTrails && this.loaded && 0 < this.tailJoints.length)
            for (let d, a, c = this.letterMinDistance, g = true, h = 0, q = 0, n, k, m, l, p, r, s = this.tailJoints[0].length - 2; 0 <= s; s--) {
  ...
  , Class_UI_KillStatus = function() {
    ...
    this.draw = function(a) {
        d && (a.globalAlpha = g,
        a.scale((n + .2 * m) * num_scale_factor, (n + .2 * m) * num_scale_factor),
    ...
    this.processCode = function(b) {
        ...
        n = .8,
        m = "MISS",
        k = true) : 4096 == b ? (e = "REVENGE",
        s = "#dd1824",
        c = objG_assets.frames.revenge,
        n = 1,
        m = "KILL",
        k = true) : 256 == b && (e = l + " KILL STREAK!",
        s = "#a5dd11",
        c = objG_assets.frames.frenzy,
        n = .8,
        m = "",
        k = true);
        "" != e && (f = new Class_StrokeStyle(40,s,true,"#000000"),
        f.setValue(e),
        f.setStrokeWidth(10),
        d = f.render(),
  ...
  , Class_ParticleDust = function() {
    ...
    _this.x = 0;
    _this.y = 0;
    _this.z = 1 * Math.random() + .3;
    _this.size = 1.2;
    _this.opacity = .8 * Math.random() + .1;
  ...
  , Class_Warship = function() {
    ...
    this.update = function(c) {
        if (this.inGame) {
            func_isInsideBox(this.x, this.y + this.floatValue, 120) ? b || (b = true) : b = false;
            let g = func_clamp((T - this.lastUpdate) / num_global_physics_step_ms, 0, 1);
            this.prevX = this.x;
            this.prevY = this.y;
            ...
            0 > this.highlightValue && (this.highlightValue = 0));
            this.type == id_entity_warship && 1 == this.state && 0 < f && (e -= .02,
            0 > e && (e = 0),
            d -= c,
            0 > d && (d = 150,
            f -= 1,
            g = Math.random() * objG_assets.warshipImage.width - objG_assets.warshipImage.width / 2,
            h = Math.random() * objG_assets.warshipImage.height / 2 - objG_assets.warshipImage.height / 4,
            objG_animationManager.addBlast(this.x + g, this.y + h + q, .5, const_Q_0, 2)));
            2 == this.state ? (a -= .01,
            0 > a && (a = 0)) : (a += .01,
            1 < a && (a = 1));
    ...
    this.draw = function(c) {
        if (this.inGame && b)
            if (this.type == id_entity_warship && objG_assets.warshipLoaded) {
                if (objG_assets.warshipImage && !this.warshipCanvas && this.loadWarshipCanvas(),
                this.warshipCanvas) {
                    c.save();
                    c.globalAlpha = a;
                    ...
                    this.warshipContext.translate(.5 * -objG_assets.cannonImage.width, -20 - d);
                    ...
                    c.drawImage(this.warshipCanvas, -objG_assets.warshipImage.width / 2, -objG_assets.warshipImage.height / 2);
                    c.restore()
                }
            } else
                this.type == id_entity_asteroid && objG_assets.asteroidImage && (c.save(),
                c.globalAlpha = a,
                c.translate(this.x, this.y),
                c.rotate(-this.angle),
                d = (3 - this.fragment + 1) / 3,
                c.scale(d, d),
                c.drawImage(objG_assets.asteroidImage, -objG_assets.asteroidImage.width / 2, -objG_assets.asteroidImage.height / 2),
                0 < this.highlightValue && (c.globalAlpha = .8 * this.highlightValue,
                c.drawImage(objG_assets.whiteAsteroidImage, -objG_assets.asteroidImage.width / 2, -objG_assets.asteroidImage.height / 2),
                c.globalAlpha = 1),
                c.restore())
    };
    this.drawReflection = function(c, d) {
        this.inGame && b && this.type == id_entity_warship && objG_assets.warshipLoaded && (c.save(),
        c.globalAlpha = .15 * a,
    ...
    this.drawInput = function(a) {};
    this.drawInfo = function(a) {
        if (this.inGame && b && this.state == const_Ic_0 && 65535 != this.energy) {
            a.save();
            a.translate(this.x, this.y);
            a.fillStyle = "rgba(126,219,226,1)";
            a.shadowOffsetX = 0;
            ...
            a.lineWidth = 1;
            let d = 28, e;
            this.type == id_entity_asteroid ? (e = 50 / this.fragment,
    ...
    this.hit = function(a) {
        func_isTimeElapsed_50ms() && (this.highlightValue = a == id_weapon_missile || this.type == id_entity_asteroid ? this.highlightValue + 1 : this.highlightValue + .3,
    ...
    this.setPose = function(a, b, d) {
        ...
        this.y = this.dstY,
        this.origAngle = this.dstAngle,
        this.first_set = false) : (a = this.dstX - this.origX,
...
window.onload = function() {
    Modernizr.canvas && Modernizr.websockets && (null == objG_followMode && (window.devicePixelRatio && (int_pixel_ratio = 1 < window.devicePixelRatio ? 2 : 1),
    objG_eventManager = new Class_EventManager,
    objG_followMode = new ClassR_FollowMode(document.getElementById("canvas")),
