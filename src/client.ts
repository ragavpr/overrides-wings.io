...
let Zb,
  ...
  Ac = 0.3,
  La = false,
  str_name_superweapon_holder: string | null = null,
  ...
  objD_pickups: Record<number, PickupItem> = {},
  objD_planes: Record<number, Plane> = {},
  objD_specialEntities: Record<number, Warship> = {},
...
class UI_GameInfo {
  ...
  draw(I: CanvasRenderingContext2D) {
    ...
    if (0 < this.#na) this.DrawWinnerLabel(I);
      ...
      this.#scale_factor != num_scale_factor &&
        ((this.#c = this.#g = null), (this.#scale_factor = num_scale_factor));
    }
  }
  ...
  DrawWarmupTime(ctx: CanvasRenderingContext2D) {
    ...
    if (this.#M) {
      ...
      ctx.globalAlpha = 1;
      ctx.drawImage(this.#N, canvas.width - d - 20, c);
      ctx.drawImage(
        this.#da,
  ...
  DrawEventLabel(ctx: CanvasRenderingContext2D) {
    ...
    if (this.#M) {
      ...
      ctx.globalAlpha = 1;
      ctx.drawImage(this.#N, canvas.width / 2 - this.#N.width / 2, c);
      ctx.drawImage(this.#M, canvas.width / 2 - this.#M.width / 2, c + 30);
    }
  }
  DrawWinnerLabel(ctx: CanvasRenderingContext2D) {
    ...
    if (this.#ha) {
      ...
      ctx.globalAlpha = 1;
      ctx.drawImage(this.#ha, canvas.width / 2 - this.#ha.width / 2, c);
      ctx.drawImage(
  ...
  DrawWarshipDestroyedLabel(ctx: CanvasRenderingContext2D) {
    ...
    if (this.#ma) {
      ...
      ctx.globalAlpha = 1;
      ctx.drawImage(this.#ma, canvas.width / 2 - this.#ma.width / 2, c);
  ...
  refreshLeaderboard(list_leaderboard_names: string[]) {
    this.#F = list_leaderboard_names;
    this.#ta = true;
  }
...
class UI_ActivityMessages {
  ...
  addActivityMessage(message_str: string) {
    ...
    for (let max = 0, d, i = 0; i < this.#b.length; i++) {
      d = this.#b[i].width + 5 + 5;
      d > max && (max = d);
    }
  }
}
class Plane {
  ...
  laserFrame = 0;
  laserTimer = 0;
  #obj_particleManager: ParticleManager | null;
  #k = 0;
  #m = 0;
  #l = 1;
  #y = 0;
  ...
  setPose(x_gu: number, y_gu: number, angle: number, h: never) {
    this.origX = this.x;
    this.origY = this.y;
...
class WS_Connection {
    ...
    for (;;) {
      ...
      if (0 == k) {
            ...
            if (m) {
              ...
              u
                ...
                : ((objG_player_plane && p == objG_player_plane.id) ||
                    ...
                    (t = 1 - u / num_sound_max_distance)),
                  0.01 < t &&
                    objG_sfxManager.playSound(str_sfxid_mlaunch, t, 1, w, null),
                  m.ammo--);
            }
          }
        ...
        if (
          162 == uint8_message_type ||
          178 == uint8_message_type ||
          163 == uint8_message_type ||
          179 == uint8_message_type ||
          169 == uint8_message_type ||
          172 == uint8_message_type
        )
          for (l = h; ; ) {
            bool_something = dataview_message.getUint32(l, true);
            l += 4;
            ...
            l += 4;
            n = dataview_message.getFloat32(l, true);
            l += 4;
            k = dataview_message.getUint16(l, true);
            l += 2;
      ...
      if ((162 != uint8_message_type && 178 != uint8_message_type) || !y)
        ...
        h += 1;
        R = dataview_message.getUint8(h);
        h += 1;
        S = dataview_message.getUint32(h, true);
        h += 4;
        for (O = ""; ; ) {
          G = dataview_message.getUint16(h, true);
          h += 2;
          if (0 == G) break;
    ...
    if (wa || int_pathMobile) {
      ...
      if (bool_isHttps) {
        (b = objG_wsConnection.remoteHost!.split(":")[0].split(".")),
          (a = Math.floor(objG_wsConnection.roomNumber) + 8079 + 1e3),
          (b =
            "wss://ip-" +
            b[0] +
            "-" +
  ...
  processMessage(array_buffer: ArrayBuffer) {
    ...
    let d = new DataView(array_buffer);
    a = d.getUint8(0);
    if (0 != a)
      if (160 == a) {
        (c = 1),
                  ...
                  (5 == h
                    ...
                    : 4 == h
                      ...
                      : 1 == h
                        ...
                        : 6 == h
                          ? objGUI_gameInfo.addMessage("Exploded!", false, null)
                          : 7 == h
                            ? objGUI_gameInfo.addMessage(
                                "KAMIKAZE!!",
                                false,
...
class SFXmanager {
  ...
  #f: SFXmanager.SoundSpriteDefinitions = {
  ...
  playSound(sfx_name: string, volume: number, speed: number, max_parallel: number, callback: null | ((nodeId: number) => void)) {
    if (this.#b && 0 != num_setting_muteVol && wa) {
      ...
      if(!(0 < max_parallel && this.#e[sfx_name] >= max_parallel)) {
        ...
        max_parallel != const_Q_0 && this.#e[sfx_name]++;
        setTimeout(() => {
          max_parallel != const_Q_0 && this.#e[sfx_name]--;
        }, q[1]);
      }
